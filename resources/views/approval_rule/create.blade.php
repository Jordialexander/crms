...
<div class="flex items-start justify-between mb-5">
    <div>
        <h1 class="text-[1.05rem] font-bold text-slate-900 leading-tight">Tambah Approval Rule</h1>
        <p class="text-[0.78rem] text-slate-400 mt-0.5">Buat rule baru untuk jenjang approval</p>
    </div>
...
<form method="POST" action="{{ route('approval-rule.store') }}">
    @csrf
    ...
    <div class="flex gap-2 mt-4">
        <button type="submit"
            ...
            <i class="bi bi-save text-xs leading-none"></i> Simpan
        </button>
        <a href="{{ route('approval-rule.index') }}"
